...
namespace Hl7.Fhir.CustomSerializer
{
    ...
    {
		public void Parse(Hl7.Fhir.Model.AuditEvent.DetailComponent result, XmlReader reader, OperationOutcome outcome, string locationPath)
		{
				...
				if (reader.IsStartElement())
				{
					switch (reader.Name)
					{
						...
						case "type":
							...
							Parse(result.TypeElement as Hl7.Fhir.Model.FhirString, reader, outcome, locationPath + ".type"); // 40
							break;
						case "valueString":
		...
		public async System.Threading.Tasks.Task ParseAsync(Hl7.Fhir.Model.AuditEvent.DetailComponent result, XmlReader reader, OperationOutcome outcome, string locationPath)
		{
			// skip ignored elements
			while (ShouldSkipNodeType(reader.NodeType))
			...
			while (await reader.ReadAsync().ConfigureAwait(false))
			{
				if (reader.IsStartElement())
				{
					switch (reader.Name)
					{
						...
						case "type":
							result.TypeElement = new Hl7.Fhir.Model.FhirString();
							await ParseAsync(result.TypeElement as Hl7.Fhir.Model.FhirString, reader, outcome, locationPath + ".type"); // 40
							break;
						case "valueString":
							result.Value = new Hl7.Fhir.Model.FhirString();
