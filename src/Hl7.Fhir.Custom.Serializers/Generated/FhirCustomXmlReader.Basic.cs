...
namespace Hl7.Fhir.CustomSerializer
{
    ...
    {
		private void Parse(Basic result, XmlReader reader, OperationOutcome outcome, string locationPath)
		{
				...
				if (reader.IsStartElement())
				{
					switch (reader.Name)
					{
						...
						case "subject":
							...
							Parse(result.Subject as Hl7.Fhir.Model.ResourceReference, reader, outcome, locationPath + ".subject"); // 110
							break;
						case "created":
							result.CreatedElement = new Hl7.Fhir.Model.Date();
							Parse(result.CreatedElement as Hl7.Fhir.Model.Date, reader, outcome, locationPath + ".created"); // 120
							break;
						case "author":
		...
		private async System.Threading.Tasks.Task ParseAsync(Basic result, XmlReader reader, OperationOutcome outcome, string locationPath)
		{
			// skip ignored elements
			while (ShouldSkipNodeType(reader.NodeType))
			...
			while (await reader.ReadAsync().ConfigureAwait(false))
			{
				if (reader.IsStartElement())
				{
					switch (reader.Name)
					{
						...
						case "subject":
							result.Subject = new Hl7.Fhir.Model.ResourceReference();
							await ParseAsync(result.Subject as Hl7.Fhir.Model.ResourceReference, reader, outcome, locationPath + ".subject"); // 110
							break;
						case "created":
							result.CreatedElement = new Hl7.Fhir.Model.Date();
							await ParseAsync(result.CreatedElement as Hl7.Fhir.Model.Date, reader, outcome, locationPath + ".created"); // 120
							break;
						case "author":
							result.Author = new Hl7.Fhir.Model.ResourceReference();
