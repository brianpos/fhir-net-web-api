...
namespace Hl7.Fhir.CustomSerializer
{
    ...
    {
		public void Parse(Hl7.Fhir.Model.BiologicallyDerivedProduct.ProcessingComponent result, XmlReader reader, OperationOutcome outcome, string locationPath, CancellationToken cancellationToken)
		{
				...
				if (reader.IsStartElement())
				{
					switch (reader.Name)
					{
						...
						case "additive":
							...
							Parse(result.Additive as Hl7.Fhir.Model.ResourceReference, reader, outcome, locationPath + ".additive", cancellationToken); // 60
							break;
						case "timeDateTime":
		...
		public async System.Threading.Tasks.Task ParseAsync(Hl7.Fhir.Model.BiologicallyDerivedProduct.ProcessingComponent result, XmlReader reader, OperationOutcome outcome, string locationPath, CancellationToken cancellationToken)
		{
			// skip ignored elements
			while (ShouldSkipNodeType(reader.NodeType))
			...
			while (await reader.ReadAsync().ConfigureAwait(false))
			{
				...
				if (reader.IsStartElement())
				{
					switch (reader.Name)
					{
						...
						case "additive":
							result.Additive = new Hl7.Fhir.Model.ResourceReference();
							await ParseAsync(result.Additive as Hl7.Fhir.Model.ResourceReference, reader, outcome, locationPath + ".additive", cancellationToken); // 60
							break;
						case "timeDateTime":
							result.Time = new Hl7.Fhir.Model.FhirDateTime();
