...
namespace Hl7.Fhir.CustomSerializer
{
    ...
    {
		private void Parse(CareTeam result, XmlReader reader, OperationOutcome outcome, string locationPath)
		{
				...
				if (reader.IsStartElement())
				{
					switch (reader.Name)
					{
						...
						case "status":
							...
							Parse(result.StatusElement as Hl7.Fhir.Model.Code<Hl7.Fhir.Model.CareTeam.CareTeamStatus>, reader, outcome, locationPath + ".status"); // 100
							break;
						case "category":
						...
						case "managingOrganization":
							...
							Parse(newItem_managingOrganization, reader, outcome, locationPath + ".managingOrganization["+result.ManagingOrganization.Count+"]"); // 190
		...
		private async System.Threading.Tasks.Task ParseAsync(CareTeam result, XmlReader reader, OperationOutcome outcome, string locationPath)
		{
			// skip ignored elements
			while (ShouldSkipNodeType(reader.NodeType))
			...
			while (await reader.ReadAsync().ConfigureAwait(false))
			{
				if (reader.IsStartElement())
				{
					switch (reader.Name)
					{
						...
						case "status":
							result.StatusElement = new Hl7.Fhir.Model.Code<Hl7.Fhir.Model.CareTeam.CareTeamStatus>();
							await ParseAsync(result.StatusElement as Hl7.Fhir.Model.Code<Hl7.Fhir.Model.CareTeam.CareTeamStatus>, reader, outcome, locationPath + ".status"); // 100
							break;
						case "category":
							var newItem_category = new Hl7.Fhir.Model.CodeableConcept();
						...
						case "managingOrganization":
							var newItem_managingOrganization = new Hl7.Fhir.Model.ResourceReference();
							await ParseAsync(newItem_managingOrganization, reader, outcome, locationPath + ".managingOrganization["+result.ManagingOrganization.Count+"]"); // 190
							result.ManagingOrganization.Add(newItem_managingOrganization);
							break;
						case "telecom":
							var newItem_telecom = new Hl7.Fhir.Model.ContactPoint();
