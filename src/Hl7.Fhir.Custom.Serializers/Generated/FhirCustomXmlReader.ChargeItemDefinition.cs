...
namespace Hl7.Fhir.CustomSerializer
{
    ...
    {
		private void Parse(ChargeItemDefinition result, XmlReader reader, OperationOutcome outcome, string locationPath, CancellationToken cancellationToken)
		{
				...
				if (reader.IsStartElement())
				{
					switch (reader.Name)
					{
						...
						case "derivedFromUri":
							...
							Parse(newItem_derivedFromUri, reader, outcome, locationPath + ".derivedFromUri["+result.DerivedFromUriElement.Count+"]", cancellationToken); // 130
							result.DerivedFromUriElement.Add(newItem_derivedFromUri);
							break;
						case "partOf":
							var newItem_partOf = new Hl7.Fhir.Model.Canonical();
							Parse(newItem_partOf, reader, outcome, locationPath + ".partOf["+result.PartOfElement.Count+"]", cancellationToken); // 140
							result.PartOfElement.Add(newItem_partOf);
							break;
						case "replaces":
							var newItem_replaces = new Hl7.Fhir.Model.Canonical();
							Parse(newItem_replaces, reader, outcome, locationPath + ".replaces["+result.ReplacesElement.Count+"]", cancellationToken); // 150
						...
						case "code":
							...
							Parse(result.Code as Hl7.Fhir.Model.CodeableConcept, reader, outcome, locationPath + ".code", cancellationToken); // 280
							break;
						case "instance":
							var newItem_instance = new Hl7.Fhir.Model.ResourceReference();
							Parse(newItem_instance, reader, outcome, locationPath + ".instance["+result.Instance.Count+"]", cancellationToken); // 290
						...
						case "applicability":
							...
							Parse(newItem_applicability, reader, outcome, locationPath + ".applicability["+result.Applicability.Count+"]", cancellationToken); // 300
						...
						case "propertyGroup":
							...
							Parse(newItem_propertyGroup, reader, outcome, locationPath + ".propertyGroup["+result.PropertyGroup.Count+"]", cancellationToken); // 310
		...
		private async System.Threading.Tasks.Task ParseAsync(ChargeItemDefinition result, XmlReader reader, OperationOutcome outcome, string locationPath, CancellationToken cancellationToken)
		{
			// skip ignored elements
			while (ShouldSkipNodeType(reader.NodeType))
			...
			while (await reader.ReadAsync().ConfigureAwait(false))
			{
				...
				if (reader.IsStartElement())
				{
					switch (reader.Name)
					{
						...
						case "derivedFromUri":
							var newItem_derivedFromUri = new Hl7.Fhir.Model.FhirUri();
							await ParseAsync(newItem_derivedFromUri, reader, outcome, locationPath + ".derivedFromUri["+result.DerivedFromUriElement.Count+"]", cancellationToken); // 130
							result.DerivedFromUriElement.Add(newItem_derivedFromUri);
							break;
						case "partOf":
							var newItem_partOf = new Hl7.Fhir.Model.Canonical();
							await ParseAsync(newItem_partOf, reader, outcome, locationPath + ".partOf["+result.PartOfElement.Count+"]", cancellationToken); // 140
							result.PartOfElement.Add(newItem_partOf);
							break;
						case "replaces":
							var newItem_replaces = new Hl7.Fhir.Model.Canonical();
							await ParseAsync(newItem_replaces, reader, outcome, locationPath + ".replaces["+result.ReplacesElement.Count+"]", cancellationToken); // 150
							result.ReplacesElement.Add(newItem_replaces);
							break;
						case "status":
							result.StatusElement = new Hl7.Fhir.Model.Code<Hl7.Fhir.Model.PublicationStatus>();
						...
						case "code":
							result.Code = new Hl7.Fhir.Model.CodeableConcept();
							await ParseAsync(result.Code as Hl7.Fhir.Model.CodeableConcept, reader, outcome, locationPath + ".code", cancellationToken); // 280
							break;
						case "instance":
							var newItem_instance = new Hl7.Fhir.Model.ResourceReference();
							await ParseAsync(newItem_instance, reader, outcome, locationPath + ".instance["+result.Instance.Count+"]", cancellationToken); // 290
							result.Instance.Add(newItem_instance);
							break;
						case "applicability":
							var newItem_applicability = new Hl7.Fhir.Model.ChargeItemDefinition.ApplicabilityComponent();
							await ParseAsync(newItem_applicability, reader, outcome, locationPath + ".applicability["+result.Applicability.Count+"]", cancellationToken); // 300
							result.Applicability.Add(newItem_applicability);
							break;
						case "propertyGroup":
							var newItem_propertyGroup = new Hl7.Fhir.Model.ChargeItemDefinition.PropertyGroupComponent();
							await ParseAsync(newItem_propertyGroup, reader, outcome, locationPath + ".propertyGroup["+result.PropertyGroup.Count+"]", cancellationToken); // 310
							result.PropertyGroup.Add(newItem_propertyGroup);
							break;
						default:
							// Property not found
