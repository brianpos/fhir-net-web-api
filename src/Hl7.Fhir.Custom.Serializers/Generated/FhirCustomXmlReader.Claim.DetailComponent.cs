...
namespace Hl7.Fhir.CustomSerializer
{
    ...
    {
		public void Parse(Hl7.Fhir.Model.Claim.DetailComponent result, XmlReader reader, OperationOutcome outcome)
		{
				...
				if (reader.IsStartElement())
				{
					switch (reader.Name)
					{
						...
						case "subDetail":
							...
							Parse(newItem_subDetail, reader, outcome); // 150
		...
		public async System.Threading.Tasks.Task ParseAsync(Hl7.Fhir.Model.Claim.DetailComponent result, XmlReader reader, OperationOutcome outcome)
		{
			// skip ignored elements
			while (ShouldSkipNodeType(reader.NodeType))
			...
			while (await reader.ReadAsync().ConfigureAwait(false))
			{
				if (reader.IsStartElement())
				{
					switch (reader.Name)
					{
						...
						case "subDetail":
							var newItem_subDetail = new Hl7.Fhir.Model.Claim.SubDetailComponent();
							await ParseAsync(newItem_subDetail, reader, outcome); // 150
							result.SubDetail.Add(newItem_subDetail);
							break;
						default:
							// Property not found
