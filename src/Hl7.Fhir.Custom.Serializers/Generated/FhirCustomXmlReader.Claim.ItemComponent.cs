...
namespace Hl7.Fhir.CustomSerializer
{
    ...
    {
		public void Parse(Hl7.Fhir.Model.Claim.ItemComponent result, XmlReader reader, OperationOutcome outcome)
		{
				...
				if (reader.IsStartElement())
				{
					switch (reader.Name)
					{
						...
						case "detail":
							...
							Parse(newItem_detail, reader, outcome); // 240
		...
		public async System.Threading.Tasks.Task ParseAsync(Hl7.Fhir.Model.Claim.ItemComponent result, XmlReader reader, OperationOutcome outcome)
		{
			// skip ignored elements
			while (ShouldSkipNodeType(reader.NodeType))
			...
			while (await reader.ReadAsync().ConfigureAwait(false))
			{
				if (reader.IsStartElement())
				{
					switch (reader.Name)
					{
						...
						case "detail":
							var newItem_detail = new Hl7.Fhir.Model.Claim.DetailComponent();
							await ParseAsync(newItem_detail, reader, outcome); // 240
							result.Detail.Add(newItem_detail);
							break;
						default:
							// Property not found
