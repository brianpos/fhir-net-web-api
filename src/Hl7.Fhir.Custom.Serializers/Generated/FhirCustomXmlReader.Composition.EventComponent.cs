...
namespace Hl7.Fhir.CustomSerializer
{
    ...
    {
		public void Parse(Hl7.Fhir.Model.Composition.EventComponent result, XmlReader reader, OperationOutcome outcome, string locationPath, CancellationToken cancellationToken)
		{
				...
				if (reader.IsStartElement())
				{
					switch (reader.Name)
					{
						...
						case "detail":
							...
							Parse(newItem_detail, reader, outcome, locationPath + ".detail["+result.Detail.Count+"]", cancellationToken); // 60
		...
		public async System.Threading.Tasks.Task ParseAsync(Hl7.Fhir.Model.Composition.EventComponent result, XmlReader reader, OperationOutcome outcome, string locationPath, CancellationToken cancellationToken)
		{
			// skip ignored elements
			while (ShouldSkipNodeType(reader.NodeType))
			...
			while (await reader.ReadAsync().ConfigureAwait(false))
			{
				...
				if (reader.IsStartElement())
				{
					switch (reader.Name)
					{
						...
						case "detail":
							var newItem_detail = new Hl7.Fhir.Model.ResourceReference();
							await ParseAsync(newItem_detail, reader, outcome, locationPath + ".detail["+result.Detail.Count+"]", cancellationToken); // 60
							result.Detail.Add(newItem_detail);
							break;
						default:
							// Property not found
