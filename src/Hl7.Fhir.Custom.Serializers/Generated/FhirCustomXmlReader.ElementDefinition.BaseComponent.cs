...
namespace Hl7.Fhir.CustomSerializer
{
    ...
    {
		public void Parse(Hl7.Fhir.Model.ElementDefinition.BaseComponent result, XmlReader reader, OperationOutcome outcome)
		{
				...
				if (reader.IsStartElement())
				{
					switch (reader.Name)
					{
						...
						case "min":
							...
							Parse(result.MinElement as Hl7.Fhir.Model.UnsignedInt, reader, outcome); // 50
							break;
						case "max":
		...
		public async System.Threading.Tasks.Task ParseAsync(Hl7.Fhir.Model.ElementDefinition.BaseComponent result, XmlReader reader, OperationOutcome outcome)
		{
			// skip ignored elements
			while (ShouldSkipNodeType(reader.NodeType))
			...
			while (await reader.ReadAsync().ConfigureAwait(false))
			{
				if (reader.IsStartElement())
				{
					switch (reader.Name)
					{
						...
						case "min":
							result.MinElement = new Hl7.Fhir.Model.UnsignedInt();
							await ParseAsync(result.MinElement as Hl7.Fhir.Model.UnsignedInt, reader, outcome); // 50
							break;
						case "max":
							result.MaxElement = new Hl7.Fhir.Model.FhirString();
