...
namespace Hl7.Fhir.CustomSerializer
{
    ...
    {
		private void Parse(EventDefinition result, XmlReader reader, OperationOutcome outcome, string locationPath)
		{
				...
				if (reader.IsStartElement())
				{
					switch (reader.Name)
					{
						...
						case "trigger":
							...
							Parse(newItem_trigger, reader, outcome, locationPath + ".trigger["+result.Trigger.Count+"]"); // 360
		...
		private async System.Threading.Tasks.Task ParseAsync(EventDefinition result, XmlReader reader, OperationOutcome outcome, string locationPath)
		{
			// skip ignored elements
			while (ShouldSkipNodeType(reader.NodeType))
			...
			while (await reader.ReadAsync().ConfigureAwait(false))
			{
				if (reader.IsStartElement())
				{
					switch (reader.Name)
					{
						...
						case "trigger":
							var newItem_trigger = new Hl7.Fhir.Model.TriggerDefinition();
							await ParseAsync(newItem_trigger, reader, outcome, locationPath + ".trigger["+result.Trigger.Count+"]"); // 360
							result.Trigger.Add(newItem_trigger);
							break;
						default:
							// Property not found
