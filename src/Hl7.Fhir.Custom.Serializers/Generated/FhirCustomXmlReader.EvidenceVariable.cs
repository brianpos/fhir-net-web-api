...
namespace Hl7.Fhir.CustomSerializer
{
    ...
    {
		private void Parse(EvidenceVariable result, XmlReader reader, OperationOutcome outcome, string locationPath)
		{
				...
				if (reader.IsStartElement())
				{
					switch (reader.Name)
					{
						...
						case "type":
							...
							Parse(result.TypeElement as Hl7.Fhir.Model.Code<Hl7.Fhir.Model.EvidenceVariableType>, reader, outcome, locationPath + ".type"); // 340
							break;
						case "characteristic":
							var newItem_characteristic = new Hl7.Fhir.Model.EvidenceVariable.CharacteristicComponent();
							Parse(newItem_characteristic, reader, outcome, locationPath + ".characteristic["+result.Characteristic.Count+"]"); // 350
		...
		private async System.Threading.Tasks.Task ParseAsync(EvidenceVariable result, XmlReader reader, OperationOutcome outcome, string locationPath)
		{
			// skip ignored elements
			while (ShouldSkipNodeType(reader.NodeType))
			...
			while (await reader.ReadAsync().ConfigureAwait(false))
			{
				if (reader.IsStartElement())
				{
					switch (reader.Name)
					{
						...
						case "type":
							result.TypeElement = new Hl7.Fhir.Model.Code<Hl7.Fhir.Model.EvidenceVariableType>();
							await ParseAsync(result.TypeElement as Hl7.Fhir.Model.Code<Hl7.Fhir.Model.EvidenceVariableType>, reader, outcome, locationPath + ".type"); // 340
							break;
						case "characteristic":
							var newItem_characteristic = new Hl7.Fhir.Model.EvidenceVariable.CharacteristicComponent();
							await ParseAsync(newItem_characteristic, reader, outcome, locationPath + ".characteristic["+result.Characteristic.Count+"]"); // 350
							result.Characteristic.Add(newItem_characteristic);
							break;
						default:
							// Property not found
