...
namespace Hl7.Fhir.CustomSerializer
{
    ...
    {
		private void Parse(ExampleScenario result, XmlReader reader, OperationOutcome outcome, string locationPath, CancellationToken cancellationToken)
		{
				...
				if (reader.IsStartElement())
				{
					switch (reader.Name)
					{
						...
						case "copyright":
							...
							Parse(result.Copyright as Hl7.Fhir.Model.Markdown, reader, outcome, locationPath + ".copyright", cancellationToken); // 200
							break;
						case "purpose":
						...
						case "actor":
							...
							Parse(newItem_actor, reader, outcome, locationPath + ".actor["+result.Actor.Count+"]", cancellationToken); // 220
							result.Actor.Add(newItem_actor);
							break;
						case "instance":
							var newItem_instance = new Hl7.Fhir.Model.ExampleScenario.InstanceComponent();
							Parse(newItem_instance, reader, outcome, locationPath + ".instance["+result.Instance.Count+"]", cancellationToken); // 230
							result.Instance.Add(newItem_instance);
							break;
						case "process":
							var newItem_process = new Hl7.Fhir.Model.ExampleScenario.ProcessComponent();
							Parse(newItem_process, reader, outcome, locationPath + ".process["+result.Process.Count+"]", cancellationToken); // 240
							result.Process.Add(newItem_process);
							break;
						case "workflow":
							var newItem_workflow = new Hl7.Fhir.Model.Canonical();
							Parse(newItem_workflow, reader, outcome, locationPath + ".workflow["+result.WorkflowElement.Count+"]", cancellationToken); // 250
		...
		private async System.Threading.Tasks.Task ParseAsync(ExampleScenario result, XmlReader reader, OperationOutcome outcome, string locationPath, CancellationToken cancellationToken)
		{
			// skip ignored elements
			while (ShouldSkipNodeType(reader.NodeType))
			...
			while (await reader.ReadAsync().ConfigureAwait(false))
			{
				...
				if (reader.IsStartElement())
				{
					switch (reader.Name)
					{
						...
						case "copyright":
							result.Copyright = new Hl7.Fhir.Model.Markdown();
							await ParseAsync(result.Copyright as Hl7.Fhir.Model.Markdown, reader, outcome, locationPath + ".copyright", cancellationToken); // 200
							break;
						case "purpose":
							result.Purpose = new Hl7.Fhir.Model.Markdown();
						...
						case "actor":
							var newItem_actor = new Hl7.Fhir.Model.ExampleScenario.ActorComponent();
							await ParseAsync(newItem_actor, reader, outcome, locationPath + ".actor["+result.Actor.Count+"]", cancellationToken); // 220
							result.Actor.Add(newItem_actor);
							break;
						case "instance":
							var newItem_instance = new Hl7.Fhir.Model.ExampleScenario.InstanceComponent();
							await ParseAsync(newItem_instance, reader, outcome, locationPath + ".instance["+result.Instance.Count+"]", cancellationToken); // 230
							result.Instance.Add(newItem_instance);
							break;
						case "process":
							var newItem_process = new Hl7.Fhir.Model.ExampleScenario.ProcessComponent();
							await ParseAsync(newItem_process, reader, outcome, locationPath + ".process["+result.Process.Count+"]", cancellationToken); // 240
							result.Process.Add(newItem_process);
							break;
						case "workflow":
							var newItem_workflow = new Hl7.Fhir.Model.Canonical();
							await ParseAsync(newItem_workflow, reader, outcome, locationPath + ".workflow["+result.WorkflowElement.Count+"]", cancellationToken); // 250
							result.WorkflowElement.Add(newItem_workflow);
							break;
						default:
							// Property not found
