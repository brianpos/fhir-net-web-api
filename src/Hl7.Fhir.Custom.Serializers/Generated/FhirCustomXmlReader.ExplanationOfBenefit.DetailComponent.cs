...
namespace Hl7.Fhir.CustomSerializer
{
    ...
    {
		public void Parse(Hl7.Fhir.Model.ExplanationOfBenefit.DetailComponent result, XmlReader reader, OperationOutcome outcome, string locationPath)
		{
				...
				if (reader.IsStartElement())
				{
					switch (reader.Name)
					{
						...
						case "subDetail":
							...
							Parse(newItem_subDetail, reader, outcome, locationPath + ".subDetail["+result.SubDetail.Count+"]"); // 170
		...
		public async System.Threading.Tasks.Task ParseAsync(Hl7.Fhir.Model.ExplanationOfBenefit.DetailComponent result, XmlReader reader, OperationOutcome outcome, string locationPath)
		{
			// skip ignored elements
			while (ShouldSkipNodeType(reader.NodeType))
			...
			while (await reader.ReadAsync().ConfigureAwait(false))
			{
				if (reader.IsStartElement())
				{
					switch (reader.Name)
					{
						...
						case "subDetail":
							var newItem_subDetail = new Hl7.Fhir.Model.ExplanationOfBenefit.SubDetailComponent();
							await ParseAsync(newItem_subDetail, reader, outcome, locationPath + ".subDetail["+result.SubDetail.Count+"]"); // 170
							result.SubDetail.Add(newItem_subDetail);
							break;
						default:
							// Property not found
