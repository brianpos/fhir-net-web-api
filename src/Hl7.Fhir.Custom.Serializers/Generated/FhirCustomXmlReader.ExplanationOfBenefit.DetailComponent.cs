...
namespace Hl7.Fhir.CustomSerializer
{
    ...
    {
		public void Parse(Hl7.Fhir.Model.ExplanationOfBenefit.DetailComponent result, XmlReader reader, OperationOutcome outcome, string locationPath, CancellationToken cancellationToken)
		{
				...
				if (reader.IsStartElement())
				{
					switch (reader.Name)
					{
						...
						case "quantity":
							...
							Parse(result.Quantity as Hl7.Fhir.Model.SimpleQuantity, reader, outcome, locationPath + ".quantity", cancellationToken); // 100
							break;
						case "unitPrice":
						...
						case "noteNumber":
							...
							Parse(newItem_noteNumber, reader, outcome, locationPath + ".noteNumber["+result.NoteNumberElement.Count+"]", cancellationToken); // 150
						...
						case "adjudication":
							...
							Parse(newItem_adjudication, reader, outcome, locationPath + ".adjudication["+result.Adjudication.Count+"]", cancellationToken); // 160
						...
						case "subDetail":
							...
							Parse(newItem_subDetail, reader, outcome, locationPath + ".subDetail["+result.SubDetail.Count+"]", cancellationToken); // 170
		...
		public async System.Threading.Tasks.Task ParseAsync(Hl7.Fhir.Model.ExplanationOfBenefit.DetailComponent result, XmlReader reader, OperationOutcome outcome, string locationPath, CancellationToken cancellationToken)
		{
			// skip ignored elements
			while (ShouldSkipNodeType(reader.NodeType))
			...
			while (await reader.ReadAsync().ConfigureAwait(false))
			{
				...
				if (reader.IsStartElement())
				{
					switch (reader.Name)
					{
						...
						case "quantity":
							result.Quantity = new Hl7.Fhir.Model.SimpleQuantity();
							await ParseAsync(result.Quantity as Hl7.Fhir.Model.SimpleQuantity, reader, outcome, locationPath + ".quantity", cancellationToken); // 100
							break;
						case "unitPrice":
							result.UnitPrice = new Hl7.Fhir.Model.Money();
						...
						case "noteNumber":
							var newItem_noteNumber = new Hl7.Fhir.Model.PositiveInt();
							await ParseAsync(newItem_noteNumber, reader, outcome, locationPath + ".noteNumber["+result.NoteNumberElement.Count+"]", cancellationToken); // 150
							result.NoteNumberElement.Add(newItem_noteNumber);
							break;
						case "adjudication":
							var newItem_adjudication = new Hl7.Fhir.Model.ExplanationOfBenefit.AdjudicationComponent();
							await ParseAsync(newItem_adjudication, reader, outcome, locationPath + ".adjudication["+result.Adjudication.Count+"]", cancellationToken); // 160
							result.Adjudication.Add(newItem_adjudication);
							break;
						case "subDetail":
							var newItem_subDetail = new Hl7.Fhir.Model.ExplanationOfBenefit.SubDetailComponent();
							await ParseAsync(newItem_subDetail, reader, outcome, locationPath + ".subDetail["+result.SubDetail.Count+"]", cancellationToken); // 170
							result.SubDetail.Add(newItem_subDetail);
							break;
						default:
							// Property not found
