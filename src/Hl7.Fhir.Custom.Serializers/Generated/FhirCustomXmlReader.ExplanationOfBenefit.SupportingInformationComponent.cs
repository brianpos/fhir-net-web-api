...
namespace Hl7.Fhir.CustomSerializer
{
    ...
    {
		public void Parse(Hl7.Fhir.Model.ExplanationOfBenefit.SupportingInformationComponent result, XmlReader reader, OperationOutcome outcome, string locationPath, CancellationToken cancellationToken)
		{
				...
				if (reader.IsStartElement())
				{
					switch (reader.Name)
					{
						...
						case "reason":
							...
							Parse(result.Reason as Hl7.Fhir.Model.Coding, reader, outcome, locationPath + ".reason", cancellationToken); // 90
							break;
						default:
							// Property not found
		...
		public async System.Threading.Tasks.Task ParseAsync(Hl7.Fhir.Model.ExplanationOfBenefit.SupportingInformationComponent result, XmlReader reader, OperationOutcome outcome, string locationPath, CancellationToken cancellationToken)
		{
			// skip ignored elements
			while (ShouldSkipNodeType(reader.NodeType))
			...
			while (await reader.ReadAsync().ConfigureAwait(false))
			{
				...
				if (reader.IsStartElement())
				{
					switch (reader.Name)
					{
						...
						case "reason":
							result.Reason = new Hl7.Fhir.Model.Coding();
							await ParseAsync(result.Reason as Hl7.Fhir.Model.Coding, reader, outcome, locationPath + ".reason", cancellationToken); // 90
							break;
						default:
							// Property not found
