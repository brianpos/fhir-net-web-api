...
namespace Hl7.Fhir.CustomSerializer
{
    ...
    {
		private void Parse(Flag result, XmlReader reader, OperationOutcome outcome, string locationPath)
		{
				...
				if (reader.IsStartElement())
				{
					switch (reader.Name)
					{
						...
						case "status":
							...
							Parse(result.StatusElement as Hl7.Fhir.Model.Code<Hl7.Fhir.Model.Flag.FlagStatus>, reader, outcome, locationPath + ".status"); // 100
							break;
						case "category":
						...
						case "author":
							...
							Parse(result.Author as Hl7.Fhir.Model.ResourceReference, reader, outcome, locationPath + ".author"); // 160
							break;
						default:
							// Property not found
		...
		private async System.Threading.Tasks.Task ParseAsync(Flag result, XmlReader reader, OperationOutcome outcome, string locationPath)
		{
			// skip ignored elements
			while (ShouldSkipNodeType(reader.NodeType))
			...
			while (await reader.ReadAsync().ConfigureAwait(false))
			{
				if (reader.IsStartElement())
				{
					switch (reader.Name)
					{
						...
						case "status":
							result.StatusElement = new Hl7.Fhir.Model.Code<Hl7.Fhir.Model.Flag.FlagStatus>();
							await ParseAsync(result.StatusElement as Hl7.Fhir.Model.Code<Hl7.Fhir.Model.Flag.FlagStatus>, reader, outcome, locationPath + ".status"); // 100
							break;
						case "category":
							var newItem_category = new Hl7.Fhir.Model.CodeableConcept();
						...
						case "author":
							result.Author = new Hl7.Fhir.Model.ResourceReference();
							await ParseAsync(result.Author as Hl7.Fhir.Model.ResourceReference, reader, outcome, locationPath + ".author"); // 160
							break;
						default:
							// Property not found
