...
namespace Hl7.Fhir.CustomSerializer
{
    ...
    {
		public void Parse(Hl7.Fhir.Model.ImplementationGuide.ManifestPageComponent result, XmlReader reader, OperationOutcome outcome, string locationPath)
		{
				...
				if (reader.IsStartElement())
				{
					switch (reader.Name)
					{
						...
						case "anchor":
							...
							Parse(newItem_anchor, reader, outcome, locationPath + ".anchor["+result.AnchorElement.Count+"]"); // 60
		...
		public async System.Threading.Tasks.Task ParseAsync(Hl7.Fhir.Model.ImplementationGuide.ManifestPageComponent result, XmlReader reader, OperationOutcome outcome, string locationPath)
		{
			// skip ignored elements
			while (ShouldSkipNodeType(reader.NodeType))
			...
			while (await reader.ReadAsync().ConfigureAwait(false))
			{
				if (reader.IsStartElement())
				{
					switch (reader.Name)
					{
						...
						case "anchor":
							var newItem_anchor = new Hl7.Fhir.Model.FhirString();
							await ParseAsync(newItem_anchor, reader, outcome, locationPath + ".anchor["+result.AnchorElement.Count+"]"); // 60
							result.AnchorElement.Add(newItem_anchor);
							break;
						default:
							// Property not found
