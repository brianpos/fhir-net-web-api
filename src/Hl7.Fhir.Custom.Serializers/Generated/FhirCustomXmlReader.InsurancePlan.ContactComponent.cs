...
namespace Hl7.Fhir.CustomSerializer
{
    ...
    {
		public void Parse(Hl7.Fhir.Model.InsurancePlan.ContactComponent result, XmlReader reader, OperationOutcome outcome, string locationPath, CancellationToken cancellationToken)
		{
				...
				if (reader.IsStartElement())
				{
					switch (reader.Name)
					{
						...
						case "purpose":
							...
							Parse(result.Purpose as Hl7.Fhir.Model.CodeableConcept, reader, outcome, locationPath + ".purpose", cancellationToken); // 40
							break;
						case "name":
		...
		public async System.Threading.Tasks.Task ParseAsync(Hl7.Fhir.Model.InsurancePlan.ContactComponent result, XmlReader reader, OperationOutcome outcome, string locationPath, CancellationToken cancellationToken)
		{
			// skip ignored elements
			while (ShouldSkipNodeType(reader.NodeType))
			...
			while (await reader.ReadAsync().ConfigureAwait(false))
			{
				...
				if (reader.IsStartElement())
				{
					switch (reader.Name)
					{
						...
						case "purpose":
							result.Purpose = new Hl7.Fhir.Model.CodeableConcept();
							await ParseAsync(result.Purpose as Hl7.Fhir.Model.CodeableConcept, reader, outcome, locationPath + ".purpose", cancellationToken); // 40
							break;
						case "name":
							result.Name = new Hl7.Fhir.Model.HumanName();
