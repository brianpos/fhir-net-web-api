...
namespace Hl7.Fhir.CustomSerializer
{
    ...
    {
		public void Parse(Hl7.Fhir.Model.Measure.StratifierComponent result, XmlReader reader, OperationOutcome outcome)
		{
				...
				if (reader.IsStartElement())
				{
					switch (reader.Name)
					{
						...
						case "criteria":
							...
							Parse(result.Criteria as Hl7.Fhir.Model.Expression, reader, outcome); // 60
							break;
						case "component":
							var newItem_component = new Hl7.Fhir.Model.Measure.ComponentComponent();
							Parse(newItem_component, reader, outcome); // 70
		...
		public async System.Threading.Tasks.Task ParseAsync(Hl7.Fhir.Model.Measure.StratifierComponent result, XmlReader reader, OperationOutcome outcome)
		{
			// skip ignored elements
			while (ShouldSkipNodeType(reader.NodeType))
			...
			while (await reader.ReadAsync().ConfigureAwait(false))
			{
				if (reader.IsStartElement())
				{
					switch (reader.Name)
					{
						...
						case "criteria":
							result.Criteria = new Hl7.Fhir.Model.Expression();
							await ParseAsync(result.Criteria as Hl7.Fhir.Model.Expression, reader, outcome); // 60
							break;
						case "component":
							var newItem_component = new Hl7.Fhir.Model.Measure.ComponentComponent();
							await ParseAsync(newItem_component, reader, outcome); // 70
							result.Component.Add(newItem_component);
							break;
						default:
							// Property not found
