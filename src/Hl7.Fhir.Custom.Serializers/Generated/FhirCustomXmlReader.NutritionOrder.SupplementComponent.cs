...
namespace Hl7.Fhir.CustomSerializer
{
    ...
    {
		public void Parse(Hl7.Fhir.Model.NutritionOrder.SupplementComponent result, XmlReader reader, OperationOutcome outcome, string locationPath, CancellationToken cancellationToken)
		{
				...
				if (reader.IsStartElement())
				{
					switch (reader.Name)
					{
						...
						case "productName":
							...
							Parse(result.ProductNameElement as Hl7.Fhir.Model.FhirString, reader, outcome, locationPath + ".productName", cancellationToken); // 50
							break;
						case "schedule":
						...
						case "instruction":
							...
							Parse(result.InstructionElement as Hl7.Fhir.Model.FhirString, reader, outcome, locationPath + ".instruction", cancellationToken); // 80
							break;
						default:
							// Property not found
		...
		public async System.Threading.Tasks.Task ParseAsync(Hl7.Fhir.Model.NutritionOrder.SupplementComponent result, XmlReader reader, OperationOutcome outcome, string locationPath, CancellationToken cancellationToken)
		{
			// skip ignored elements
			while (ShouldSkipNodeType(reader.NodeType))
			...
			while (await reader.ReadAsync().ConfigureAwait(false))
			{
				...
				if (reader.IsStartElement())
				{
					switch (reader.Name)
					{
						...
						case "productName":
							result.ProductNameElement = new Hl7.Fhir.Model.FhirString();
							await ParseAsync(result.ProductNameElement as Hl7.Fhir.Model.FhirString, reader, outcome, locationPath + ".productName", cancellationToken); // 50
							break;
						case "schedule":
							var newItem_schedule = new Hl7.Fhir.Model.Timing();
						...
						case "instruction":
							result.InstructionElement = new Hl7.Fhir.Model.FhirString();
							await ParseAsync(result.InstructionElement as Hl7.Fhir.Model.FhirString, reader, outcome, locationPath + ".instruction", cancellationToken); // 80
							break;
						default:
							// Property not found
