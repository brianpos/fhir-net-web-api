...
namespace Hl7.Fhir.CustomSerializer
{
    ...
    {
		private void Parse(OperationOutcome result, XmlReader reader, OperationOutcome outcome, string locationPath)
		{
				...
				if (reader.IsStartElement())
				{
					switch (reader.Name)
					{
						...
						case "issue":
							...
							Parse(newItem_issue, reader, outcome, locationPath + ".issue["+result.Issue.Count+"]"); // 90
		...
		private async System.Threading.Tasks.Task ParseAsync(OperationOutcome result, XmlReader reader, OperationOutcome outcome, string locationPath)
		{
			// skip ignored elements
			while (ShouldSkipNodeType(reader.NodeType))
			...
			while (await reader.ReadAsync().ConfigureAwait(false))
			{
				if (reader.IsStartElement())
				{
					switch (reader.Name)
					{
						...
						case "issue":
							var newItem_issue = new Hl7.Fhir.Model.OperationOutcome.IssueComponent();
							await ParseAsync(newItem_issue, reader, outcome, locationPath + ".issue["+result.Issue.Count+"]"); // 90
							result.Issue.Add(newItem_issue);
							break;
						default:
							// Property not found
