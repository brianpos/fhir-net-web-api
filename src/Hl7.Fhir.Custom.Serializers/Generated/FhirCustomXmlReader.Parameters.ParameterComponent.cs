...
namespace Hl7.Fhir.CustomSerializer
{
    ...
    {
		public void Parse(Hl7.Fhir.Model.Parameters.ParameterComponent result, XmlReader reader, OperationOutcome outcome)
		{
				...
				if (reader.IsStartElement())
				{
					switch (reader.Name)
					{
						...
						case "part":
							...
							Parse(newItem_part, reader, outcome); // 70
		...
		public async System.Threading.Tasks.Task ParseAsync(Hl7.Fhir.Model.Parameters.ParameterComponent result, XmlReader reader, OperationOutcome outcome)
		{
			// skip ignored elements
			while (ShouldSkipNodeType(reader.NodeType))
			...
			while (await reader.ReadAsync().ConfigureAwait(false))
			{
				if (reader.IsStartElement())
				{
					switch (reader.Name)
					{
						...
						case "part":
							var newItem_part = new Hl7.Fhir.Model.Parameters.ParameterComponent();
							await ParseAsync(newItem_part, reader, outcome); // 70
							result.Part.Add(newItem_part);
							break;
						default:
							// Property not found
