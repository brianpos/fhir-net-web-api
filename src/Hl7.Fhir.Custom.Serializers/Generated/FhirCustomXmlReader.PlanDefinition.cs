...
namespace Hl7.Fhir.CustomSerializer
{
    ...
    {
		private void Parse(PlanDefinition result, XmlReader reader, OperationOutcome outcome)
		{
		...
		private async System.Threading.Tasks.Task ParseAsync(PlanDefinition result, XmlReader reader, OperationOutcome outcome)
		{
			// skip ignored elements
			while (ShouldSkipNodeType(reader.NodeType))
			...
			while (await reader.ReadAsync().ConfigureAwait(false))
			{
				if (reader.IsStartElement())
				{
					switch (reader.Name)
					{
						...
						case "library":
							var newItem_library = new Hl7.Fhir.Model.Canonical();
							await ParseAsync(newItem_library, reader, outcome); // 370
							result.LibraryElement.Add(newItem_library);
							break;
						case "goal":
							var newItem_goal = new Hl7.Fhir.Model.PlanDefinition.GoalComponent();
							await ParseAsync(newItem_goal, reader, outcome); // 380
							result.Goal.Add(newItem_goal);
							break;
						case "action":
							var newItem_action = new Hl7.Fhir.Model.PlanDefinition.ActionComponent();
							await ParseAsync(newItem_action, reader, outcome); // 390
							result.Action.Add(newItem_action);
							break;
						default:
							// Property not found
