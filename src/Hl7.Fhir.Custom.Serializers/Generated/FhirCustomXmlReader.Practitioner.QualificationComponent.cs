...
namespace Hl7.Fhir.CustomSerializer
{
    ...
    {
		public void Parse(Hl7.Fhir.Model.Practitioner.QualificationComponent result, XmlReader reader, OperationOutcome outcome, string locationPath)
		{
				...
				if (reader.IsStartElement())
				{
					switch (reader.Name)
					{
						...
						case "issuer":
							...
							Parse(result.Issuer as Hl7.Fhir.Model.ResourceReference, reader, outcome, locationPath + ".issuer"); // 70
							break;
						default:
							// Property not found
		...
		public async System.Threading.Tasks.Task ParseAsync(Hl7.Fhir.Model.Practitioner.QualificationComponent result, XmlReader reader, OperationOutcome outcome, string locationPath)
		{
			// skip ignored elements
			while (ShouldSkipNodeType(reader.NodeType))
			...
			while (await reader.ReadAsync().ConfigureAwait(false))
			{
				if (reader.IsStartElement())
				{
					switch (reader.Name)
					{
						...
						case "issuer":
							result.Issuer = new Hl7.Fhir.Model.ResourceReference();
							await ParseAsync(result.Issuer as Hl7.Fhir.Model.ResourceReference, reader, outcome, locationPath + ".issuer"); // 70
							break;
						default:
							// Property not found
