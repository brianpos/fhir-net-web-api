...
namespace Hl7.Fhir.CustomSerializer
{
    ...
    {
		public void Parse(Hl7.Fhir.Model.Questionnaire.AnswerOptionComponent result, XmlReader reader, OperationOutcome outcome, string locationPath)
		{
				...
				if (reader.IsStartElement())
				{
					switch (reader.Name)
					{
						...
						case "initialSelected":
							...
							Parse(result.InitialSelectedElement as Hl7.Fhir.Model.FhirBoolean, reader, outcome, locationPath + ".initialSelected"); // 50
							break;
						default:
							// Property not found
		...
		public async System.Threading.Tasks.Task ParseAsync(Hl7.Fhir.Model.Questionnaire.AnswerOptionComponent result, XmlReader reader, OperationOutcome outcome, string locationPath)
		{
			// skip ignored elements
			while (ShouldSkipNodeType(reader.NodeType))
			...
			while (await reader.ReadAsync().ConfigureAwait(false))
			{
				if (reader.IsStartElement())
				{
					switch (reader.Name)
					{
						...
						case "initialSelected":
							result.InitialSelectedElement = new Hl7.Fhir.Model.FhirBoolean();
							await ParseAsync(result.InitialSelectedElement as Hl7.Fhir.Model.FhirBoolean, reader, outcome, locationPath + ".initialSelected"); // 50
							break;
						default:
							// Property not found
