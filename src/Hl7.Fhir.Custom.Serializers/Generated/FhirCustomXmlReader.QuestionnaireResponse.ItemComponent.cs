...
namespace Hl7.Fhir.CustomSerializer
{
    ...
    {
		public void Parse(Hl7.Fhir.Model.QuestionnaireResponse.ItemComponent result, XmlReader reader, OperationOutcome outcome, string locationPath, CancellationToken cancellationToken)
		{
				...
				if (reader.IsStartElement())
				{
					switch (reader.Name)
					{
						...
						case "answer":
							...
							Parse(newItem_answer, reader, outcome, locationPath + ".answer["+result.Answer.Count+"]", cancellationToken); // 70
							result.Answer.Add(newItem_answer);
							break;
						case "item":
							var newItem_item = new Hl7.Fhir.Model.QuestionnaireResponse.ItemComponent();
							Parse(newItem_item, reader, outcome, locationPath + ".item["+result.Item.Count+"]", cancellationToken); // 80
		...
		public async System.Threading.Tasks.Task ParseAsync(Hl7.Fhir.Model.QuestionnaireResponse.ItemComponent result, XmlReader reader, OperationOutcome outcome, string locationPath, CancellationToken cancellationToken)
		{
			// skip ignored elements
			while (ShouldSkipNodeType(reader.NodeType))
			...
			while (await reader.ReadAsync().ConfigureAwait(false))
			{
				...
				if (reader.IsStartElement())
				{
					switch (reader.Name)
					{
						...
						case "answer":
							var newItem_answer = new Hl7.Fhir.Model.QuestionnaireResponse.AnswerComponent();
							await ParseAsync(newItem_answer, reader, outcome, locationPath + ".answer["+result.Answer.Count+"]", cancellationToken); // 70
							result.Answer.Add(newItem_answer);
							break;
						case "item":
							var newItem_item = new Hl7.Fhir.Model.QuestionnaireResponse.ItemComponent();
							await ParseAsync(newItem_item, reader, outcome, locationPath + ".item["+result.Item.Count+"]", cancellationToken); // 80
							result.Item.Add(newItem_item);
							break;
						default:
							// Property not found
