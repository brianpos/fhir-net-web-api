...
namespace Hl7.Fhir.CustomSerializer
{
    ...
    {
		private void Parse(ResearchElementDefinition result, XmlReader reader, OperationOutcome outcome, string locationPath, CancellationToken cancellationToken)
		{
				...
				if (reader.IsStartElement())
				{
					switch (reader.Name)
					{
						...
						case "characteristic":
							...
							Parse(newItem_characteristic, reader, outcome, locationPath + ".characteristic["+result.Characteristic.Count+"]", cancellationToken); // 410
		...
		private async System.Threading.Tasks.Task ParseAsync(ResearchElementDefinition result, XmlReader reader, OperationOutcome outcome, string locationPath, CancellationToken cancellationToken)
		{
			// skip ignored elements
			while (ShouldSkipNodeType(reader.NodeType))
			...
			while (await reader.ReadAsync().ConfigureAwait(false))
			{
				...
				if (reader.IsStartElement())
				{
					switch (reader.Name)
					{
						...
						case "characteristic":
							var newItem_characteristic = new Hl7.Fhir.Model.ResearchElementDefinition.CharacteristicComponent();
							await ParseAsync(newItem_characteristic, reader, outcome, locationPath + ".characteristic["+result.Characteristic.Count+"]", cancellationToken); // 410
							result.Characteristic.Add(newItem_characteristic);
							break;
						default:
							// Property not found
