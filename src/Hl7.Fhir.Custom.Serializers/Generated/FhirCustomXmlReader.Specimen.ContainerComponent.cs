...
namespace Hl7.Fhir.CustomSerializer
{
    ...
    {
		public void Parse(Hl7.Fhir.Model.Specimen.ContainerComponent result, XmlReader reader, OperationOutcome outcome, string locationPath, CancellationToken cancellationToken)
		{
				...
				if (reader.IsStartElement())
				{
					switch (reader.Name)
					{
						...
						case "additiveCodeableConcept":
							...
							Parse(result.Additive as Hl7.Fhir.Model.CodeableConcept, reader, outcome, locationPath + ".additive", cancellationToken); // 90
							break;
						case "additiveReference":
							result.Additive = new Hl7.Fhir.Model.ResourceReference();
							Parse(result.Additive as Hl7.Fhir.Model.ResourceReference, reader, outcome, locationPath + ".additive", cancellationToken); // 90
							break;
						default:
							// Property not found
		...
		public async System.Threading.Tasks.Task ParseAsync(Hl7.Fhir.Model.Specimen.ContainerComponent result, XmlReader reader, OperationOutcome outcome, string locationPath, CancellationToken cancellationToken)
		{
			// skip ignored elements
			while (ShouldSkipNodeType(reader.NodeType))
			...
			while (await reader.ReadAsync().ConfigureAwait(false))
			{
				...
				if (reader.IsStartElement())
				{
					switch (reader.Name)
					{
						...
						case "additiveCodeableConcept":
							result.Additive = new Hl7.Fhir.Model.CodeableConcept();
							await ParseAsync(result.Additive as Hl7.Fhir.Model.CodeableConcept, reader, outcome, locationPath + ".additive", cancellationToken); // 90
							break;
						case "additiveReference":
							result.Additive = new Hl7.Fhir.Model.ResourceReference();
							await ParseAsync(result.Additive as Hl7.Fhir.Model.ResourceReference, reader, outcome, locationPath + ".additive", cancellationToken); // 90
							break;
						default:
							// Property not found
