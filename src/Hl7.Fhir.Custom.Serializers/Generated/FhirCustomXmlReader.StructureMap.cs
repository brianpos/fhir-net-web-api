...
namespace Hl7.Fhir.CustomSerializer
{
    ...
    {
		private void Parse(StructureMap result, XmlReader reader, OperationOutcome outcome, string locationPath)
		{
				...
				if (reader.IsStartElement())
				{
					switch (reader.Name)
					{
						...
						case "structure":
							...
							Parse(newItem_structure, reader, outcome, locationPath + ".structure["+result.Structure.Count+"]"); // 240
							result.Structure.Add(newItem_structure);
							break;
						case "import":
							var newItem_import = new Hl7.Fhir.Model.Canonical();
							Parse(newItem_import, reader, outcome, locationPath + ".import["+result.ImportElement.Count+"]"); // 250
		...
		private async System.Threading.Tasks.Task ParseAsync(StructureMap result, XmlReader reader, OperationOutcome outcome, string locationPath)
		{
			// skip ignored elements
			while (ShouldSkipNodeType(reader.NodeType))
			...
			while (await reader.ReadAsync().ConfigureAwait(false))
			{
				if (reader.IsStartElement())
				{
					switch (reader.Name)
					{
						...
						case "structure":
							var newItem_structure = new Hl7.Fhir.Model.StructureMap.StructureComponent();
							await ParseAsync(newItem_structure, reader, outcome, locationPath + ".structure["+result.Structure.Count+"]"); // 240
							result.Structure.Add(newItem_structure);
							break;
						case "import":
							var newItem_import = new Hl7.Fhir.Model.Canonical();
							await ParseAsync(newItem_import, reader, outcome, locationPath + ".import["+result.ImportElement.Count+"]"); // 250
							result.ImportElement.Add(newItem_import);
							break;
						case "group":
							var newItem_group = new Hl7.Fhir.Model.StructureMap.GroupComponent();
