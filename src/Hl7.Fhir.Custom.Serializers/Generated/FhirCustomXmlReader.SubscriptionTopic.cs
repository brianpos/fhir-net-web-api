...
namespace Hl7.Fhir.CustomSerializer
{
    ...
    {
		private void Parse(SubscriptionTopic result, XmlReader reader, OperationOutcome outcome, string locationPath, CancellationToken cancellationToken)
		{
				...
				if (reader.IsStartElement())
				{
					switch (reader.Name)
					{
						...
						case "derivedFrom":
							...
							Parse(newItem_derivedFrom, reader, outcome, locationPath + ".derivedFrom["+result.DerivedFromElement.Count+"]", cancellationToken); // 130
						...
						case "resourceTrigger":
							var newItem_resourceTrigger = new Hl7.Fhir.Model.SubscriptionTopic.ResourceTriggerComponent();
							Parse(newItem_resourceTrigger, reader, outcome, locationPath + ".resourceTrigger["+result.ResourceTrigger.Count+"]", cancellationToken); // 270
							result.ResourceTrigger.Add(newItem_resourceTrigger);
							break;
						case "eventTrigger":
							var newItem_eventTrigger = new Hl7.Fhir.Model.SubscriptionTopic.EventTriggerComponent();
							Parse(newItem_eventTrigger, reader, outcome, locationPath + ".eventTrigger["+result.EventTrigger.Count+"]", cancellationToken); // 280
							result.EventTrigger.Add(newItem_eventTrigger);
							break;
						case "canFilterBy":
							var newItem_canFilterBy = new Hl7.Fhir.Model.SubscriptionTopic.CanFilterByComponent();
							Parse(newItem_canFilterBy, reader, outcome, locationPath + ".canFilterBy["+result.CanFilterBy.Count+"]", cancellationToken); // 290
							result.CanFilterBy.Add(newItem_canFilterBy);
							break;
						case "notificationShape":
							var newItem_notificationShape = new Hl7.Fhir.Model.SubscriptionTopic.NotificationShapeComponent();
							Parse(newItem_notificationShape, reader, outcome, locationPath + ".notificationShape["+result.NotificationShape.Count+"]", cancellationToken); // 300
							result.NotificationShape.Add(newItem_notificationShape);
							break;
						default:
							// Property not found
		...
		private async System.Threading.Tasks.Task ParseAsync(SubscriptionTopic result, XmlReader reader, OperationOutcome outcome, string locationPath, CancellationToken cancellationToken)
		{
			// skip ignored elements
			while (ShouldSkipNodeType(reader.NodeType))
			...
			while (await reader.ReadAsync().ConfigureAwait(false))
			{
				...
				if (reader.IsStartElement())
				{
					switch (reader.Name)
					{
						...
						case "derivedFrom":
							var newItem_derivedFrom = new Hl7.Fhir.Model.Canonical();
							await ParseAsync(newItem_derivedFrom, reader, outcome, locationPath + ".derivedFrom["+result.DerivedFromElement.Count+"]", cancellationToken); // 130
							result.DerivedFromElement.Add(newItem_derivedFrom);
							break;
						case "status":
							result.StatusElement = new Hl7.Fhir.Model.Code<Hl7.Fhir.Model.PublicationStatus>();
						...
						case "resourceTrigger":
							var newItem_resourceTrigger = new Hl7.Fhir.Model.SubscriptionTopic.ResourceTriggerComponent();
							await ParseAsync(newItem_resourceTrigger, reader, outcome, locationPath + ".resourceTrigger["+result.ResourceTrigger.Count+"]", cancellationToken); // 270
							result.ResourceTrigger.Add(newItem_resourceTrigger);
							break;
						case "eventTrigger":
							var newItem_eventTrigger = new Hl7.Fhir.Model.SubscriptionTopic.EventTriggerComponent();
							await ParseAsync(newItem_eventTrigger, reader, outcome, locationPath + ".eventTrigger["+result.EventTrigger.Count+"]", cancellationToken); // 280
							result.EventTrigger.Add(newItem_eventTrigger);
							break;
						case "canFilterBy":
							var newItem_canFilterBy = new Hl7.Fhir.Model.SubscriptionTopic.CanFilterByComponent();
							await ParseAsync(newItem_canFilterBy, reader, outcome, locationPath + ".canFilterBy["+result.CanFilterBy.Count+"]", cancellationToken); // 290
							result.CanFilterBy.Add(newItem_canFilterBy);
							break;
						case "notificationShape":
							var newItem_notificationShape = new Hl7.Fhir.Model.SubscriptionTopic.NotificationShapeComponent();
							await ParseAsync(newItem_notificationShape, reader, outcome, locationPath + ".notificationShape["+result.NotificationShape.Count+"]", cancellationToken); // 300
							result.NotificationShape.Add(newItem_notificationShape);
							break;
						default:
							// Property not found
