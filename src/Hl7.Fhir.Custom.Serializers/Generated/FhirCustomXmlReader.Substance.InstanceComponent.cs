...
namespace Hl7.Fhir.CustomSerializer
{
    ...
    {
		public void Parse(Hl7.Fhir.Model.Substance.InstanceComponent result, XmlReader reader, OperationOutcome outcome, string locationPath)
		{
				...
				if (reader.IsStartElement())
				{
					switch (reader.Name)
					{
						...
						case "expiry":
							...
							Parse(result.ExpiryElement as Hl7.Fhir.Model.FhirDateTime, reader, outcome, locationPath + ".expiry"); // 50
							break;
						case "quantity":
		...
		public async System.Threading.Tasks.Task ParseAsync(Hl7.Fhir.Model.Substance.InstanceComponent result, XmlReader reader, OperationOutcome outcome, string locationPath)
		{
			// skip ignored elements
			while (ShouldSkipNodeType(reader.NodeType))
			...
			while (await reader.ReadAsync().ConfigureAwait(false))
			{
				if (reader.IsStartElement())
				{
					switch (reader.Name)
					{
						...
						case "expiry":
							result.ExpiryElement = new Hl7.Fhir.Model.FhirDateTime();
							await ParseAsync(result.ExpiryElement as Hl7.Fhir.Model.FhirDateTime, reader, outcome, locationPath + ".expiry"); // 50
							break;
						case "quantity":
							result.Quantity = new Hl7.Fhir.Model.SimpleQuantity();
