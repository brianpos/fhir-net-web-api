...
namespace Hl7.Fhir.CustomSerializer
{
    ...
    {
		private void Parse(SubstanceProtein result, XmlReader reader, OperationOutcome outcome, string locationPath)
		{
				...
				if (reader.IsStartElement())
				{
					switch (reader.Name)
					{
						...
						case "disulfideLinkage":
							...
							Parse(newItem_disulfideLinkage, reader, outcome, locationPath + ".disulfideLinkage["+result.DisulfideLinkageElement.Count+"]"); // 110
						...
						case "subunit":
							...
							Parse(newItem_subunit, reader, outcome, locationPath + ".subunit["+result.Subunit.Count+"]"); // 120
		...
		private async System.Threading.Tasks.Task ParseAsync(SubstanceProtein result, XmlReader reader, OperationOutcome outcome, string locationPath)
		{
			// skip ignored elements
			while (ShouldSkipNodeType(reader.NodeType))
			...
			while (await reader.ReadAsync().ConfigureAwait(false))
			{
				if (reader.IsStartElement())
				{
					switch (reader.Name)
					{
						...
						case "disulfideLinkage":
							var newItem_disulfideLinkage = new Hl7.Fhir.Model.FhirString();
							await ParseAsync(newItem_disulfideLinkage, reader, outcome, locationPath + ".disulfideLinkage["+result.DisulfideLinkageElement.Count+"]"); // 110
							result.DisulfideLinkageElement.Add(newItem_disulfideLinkage);
							break;
						case "subunit":
							var newItem_subunit = new Hl7.Fhir.Model.SubstanceProtein.SubunitComponent();
							await ParseAsync(newItem_subunit, reader, outcome, locationPath + ".subunit["+result.Subunit.Count+"]"); // 120
							result.Subunit.Add(newItem_subunit);
							break;
						default:
							// Property not found
