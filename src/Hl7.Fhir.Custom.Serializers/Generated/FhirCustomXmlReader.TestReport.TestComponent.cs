...
namespace Hl7.Fhir.CustomSerializer
{
    ...
    {
		public void Parse(Hl7.Fhir.Model.TestReport.TestComponent result, XmlReader reader, OperationOutcome outcome)
		{
				...
				if (reader.IsStartElement())
				{
					switch (reader.Name)
					{
						...
						case "action":
							...
							Parse(newItem_action, reader, outcome); // 60
		...
		public async System.Threading.Tasks.Task ParseAsync(Hl7.Fhir.Model.TestReport.TestComponent result, XmlReader reader, OperationOutcome outcome)
		{
			// skip ignored elements
			while (ShouldSkipNodeType(reader.NodeType))
			...
			while (await reader.ReadAsync().ConfigureAwait(false))
			{
				if (reader.IsStartElement())
				{
					switch (reader.Name)
					{
						...
						case "action":
							var newItem_action = new Hl7.Fhir.Model.TestReport.TestActionComponent();
							await ParseAsync(newItem_action, reader, outcome); // 60
							result.Action.Add(newItem_action);
							break;
						default:
							// Property not found
