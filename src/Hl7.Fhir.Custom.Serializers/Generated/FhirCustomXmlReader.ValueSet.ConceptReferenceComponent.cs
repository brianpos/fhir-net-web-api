...
namespace Hl7.Fhir.CustomSerializer
{
    ...
    {
		public void Parse(Hl7.Fhir.Model.ValueSet.ConceptReferenceComponent result, XmlReader reader, OperationOutcome outcome)
		{
				...
				if (reader.IsStartElement())
				{
					switch (reader.Name)
					{
						...
						case "designation":
							...
							Parse(newItem_designation, reader, outcome); // 60
		...
		public async System.Threading.Tasks.Task ParseAsync(Hl7.Fhir.Model.ValueSet.ConceptReferenceComponent result, XmlReader reader, OperationOutcome outcome)
		{
			// skip ignored elements
			while (ShouldSkipNodeType(reader.NodeType))
			...
			while (await reader.ReadAsync().ConfigureAwait(false))
			{
				if (reader.IsStartElement())
				{
					switch (reader.Name)
					{
						...
						case "designation":
							var newItem_designation = new Hl7.Fhir.Model.ValueSet.DesignationComponent();
							await ParseAsync(newItem_designation, reader, outcome); // 60
							result.Designation.Add(newItem_designation);
							break;
						default:
							// Property not found
