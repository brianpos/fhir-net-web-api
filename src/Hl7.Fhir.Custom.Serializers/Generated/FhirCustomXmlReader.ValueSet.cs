...
namespace Hl7.Fhir.CustomSerializer
{
    ...
    {
		private void Parse(ValueSet result, XmlReader reader, OperationOutcome outcome, string locationPath, CancellationToken cancellationToken)
		{
				...
				if (reader.IsStartElement())
				{
					switch (reader.Name)
					{
						...
						case "immutable":
							...
							Parse(result.ImmutableElement as Hl7.Fhir.Model.FhirBoolean, reader, outcome, locationPath + ".immutable", cancellationToken); // 220
							break;
						case "purpose":
						...
						case "compose":
							...
							Parse(result.Compose as Hl7.Fhir.Model.ValueSet.ComposeComponent, reader, outcome, locationPath + ".compose", cancellationToken); // 250
							break;
						case "expansion":
							result.Expansion = new Hl7.Fhir.Model.ValueSet.ExpansionComponent();
							Parse(result.Expansion as Hl7.Fhir.Model.ValueSet.ExpansionComponent, reader, outcome, locationPath + ".expansion", cancellationToken); // 260
							break;
						default:
							// Property not found
		...
		private async System.Threading.Tasks.Task ParseAsync(ValueSet result, XmlReader reader, OperationOutcome outcome, string locationPath, CancellationToken cancellationToken)
		{
			// skip ignored elements
			while (ShouldSkipNodeType(reader.NodeType))
			...
			while (await reader.ReadAsync().ConfigureAwait(false))
			{
				...
				if (reader.IsStartElement())
				{
					switch (reader.Name)
					{
						...
						case "immutable":
							result.ImmutableElement = new Hl7.Fhir.Model.FhirBoolean();
							await ParseAsync(result.ImmutableElement as Hl7.Fhir.Model.FhirBoolean, reader, outcome, locationPath + ".immutable", cancellationToken); // 220
							break;
						case "purpose":
							result.Purpose = new Hl7.Fhir.Model.Markdown();
						...
						case "compose":
							result.Compose = new Hl7.Fhir.Model.ValueSet.ComposeComponent();
							await ParseAsync(result.Compose as Hl7.Fhir.Model.ValueSet.ComposeComponent, reader, outcome, locationPath + ".compose", cancellationToken); // 250
							break;
						case "expansion":
							result.Expansion = new Hl7.Fhir.Model.ValueSet.ExpansionComponent();
							await ParseAsync(result.Expansion as Hl7.Fhir.Model.ValueSet.ExpansionComponent, reader, outcome, locationPath + ".expansion", cancellationToken); // 260
							break;
						default:
							// Property not found
